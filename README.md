...
- 结构化翻译上下文：翻译请求会注入位置、风险、命中术语、同文件邻近文本和同文件 TM 示例。
...
`--glossary` 和 `--memory` 会参与 provider 的结构化上下文包：exact TM 命中会直接复用译文；未命中时，命中术语、同文件邻近文本和同文件 TM 示例会进入 `TranslationRequest.context`。
