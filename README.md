...
- 可插拔翻译 provider：默认 `dry-run` 可测试，`openai` provider 作为 GPT 兜底入口，`openai-chat` / `qwen-mt` 可接入 OpenAI-compatible Chat Completions 端点做模型赛马。
- 结构化翻译上下文：翻译请求会注入位置、风险、命中术语、同文件邻近文本、同文件 TM、相似 TM 示例和世界观片段。
- 候选译文缓存、request log 与 trace：provider 产物按 provider、source hash、context hash 和 glossary hash 缓存；每次 provider 调用可落 JSONL，记录入参、译文、响应模型、响应 id 和 token 用量；每条译文记录 provenance，区分 state、TM、cache 和 provider。
...
- 术语候选二次提炼：`terms extract` 召回候选后，`terms refine` 用 `rules` 或 `openai` provider 输出本轮 refined 结果，并可通过持久 refined cache 复用已提炼术语；审校包供人工确认，审校通过后写入本地 glossary cache。
...
DASHSCOPE_API_KEY=... python3 -m limbus_translate.cli eval compare \
  --gold cache/eval/gold-curated.json \
  --provider baseline=dry-run \
  --provider qwen=qwen-mt:qwen-mt-plus \
  --report build/eval-compare-report.json
