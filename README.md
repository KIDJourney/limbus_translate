...
`workflow run` 会串联 scan、TM 构建、可选 glossary audit、可选 lore 导入/索引、同结构翻译输出、QA 和翻译审校包，工作目录中会写出 `missing-units.json`、`tm.json`、可选 `glossary-audit.json`、可选 `lore.json` / `lore-index.json`、`qa-report.json`、`translation-review/` 和 `summary.json`。`summary.json` 记录待译单元数、实际写入数、缺译原因分布、术语库审计、QA 汇总和所有产物路径，适合作为一次上游更新的交接入口。
