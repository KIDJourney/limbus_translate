.PHONY: validate-docs test smoke sync-glossary check-provider-env prepare-current-localize-review finalize-current-localize-review apply-current-term-review prepare-current-model-eval apply-current-model-review compare-current-models reproduce-current-localize publish-current-localize-artifact

validate-docs:
	./scripts/validate-docs.sh

test:
	python3 -c "from tests.test_scanner import test_scan_missing_detects_korean_target_and_blank_target, test_scan_aligns_data_list_by_id_when_order_changes, test_scan_reports_missing_data_list_record, test_scan_policy_includes_non_default_text_path, test_scan_policy_excludes_noise_path, test_scan_can_limit_to_changed_files, test_scan_can_limit_to_changed_source_paths_and_marks_source_changed, test_read_changed_files_normalizes_repo_paths; from tests.test_evaluation import test_gold_evaluation_passes_matching_provider, test_gold_evaluation_reports_quality_issues, test_eval_report_roundtrip, test_eval_comparison_ranks_providers, test_gold_evaluation_reuses_candidate_cache_and_logs_requests, test_eval_comparison_cache_key_uses_provider_spec_not_label, test_sample_gold_cases_is_stratified_and_repeatable, test_build_gold_cases_from_reference_tree, test_write_gold_cases_roundtrip, test_write_gold_review_pack_exports_review_files, test_apply_gold_review_csv_preserves_original_case_structure; from tests.test_glossary import test_match_terms_uses_source_and_variants, test_audit_terms_reports_conflicts_and_invalid_rows, test_merge_glossary_terms_later_inputs_override_same_source; from tests.test_localize import test_prepare_localize_update_writes_changed_files_and_source_baseline; from tests.test_lore import test_import_markdown_lore_cache_roundtrip_and_match, test_import_json_lore_uses_anchors, test_import_lore_directory_skips_readme, test_match_lore_uses_ngram_similarity_without_anchor_hit, test_lore_index_roundtrip_and_search; from tests.test_memory import test_evaluate_memory_retrieval_scores_fuzzy_matches_and_thresholds; from tests.test_qa import test_qa_detects_placeholder_mismatch, test_qa_detects_traditional_and_length, test_qa_uses_path_specific_length_policy, test_qa_uses_display_width_policy; from tests.test_review import test_translation_review_pack_exports_qa_and_apply_review_state; from tests.test_terms import test_extract_term_candidates_excludes_known_glossary, test_rules_refiner_classifies_core_decisions, test_refined_terms_cache_roundtrip, test_refine_candidates_with_cache_reuses_existing_terms, test_get_term_refiner_resolves_supported_providers, test_promote_refined_terms_exports_only_confirmed_terms, test_write_term_review_pack_exports_review_and_paratranz_files, test_glossary_terms_from_review_csv_imports_only_approved_rows; from tests.test_translator import test_translate_appends_missing_data_list_record, test_translate_reuses_candidate_cache_and_records_trace; from tests.test_state import test_translate_skips_locked_unit, test_state_apply_writes_reviewed_translation_without_provider, test_state_status_reports_ready_and_pending_units; from tests.test_context import test_translate_provider_receives_structured_context, test_context_includes_cross_file_similar_memory, test_source_changed_context_includes_previous_target_text; from tests.test_providers import test_openai_compatible_chat_provider_sends_structured_prompt, test_qwen_mt_provider_uses_translation_options_without_system_message, test_qwen_translation_options_tolerates_invalid_context, test_get_provider_accepts_compatible_provider_specs, test_get_provider_rejects_missing_compatible_model; test_scan_missing_detects_korean_target_and_blank_target(); test_scan_aligns_data_list_by_id_when_order_changes(); test_scan_reports_missing_data_list_record(); test_scan_policy_includes_non_default_text_path(); test_scan_policy_excludes_noise_path(); test_scan_can_limit_to_changed_files(); test_scan_can_limit_to_changed_source_paths_and_marks_source_changed(); test_read_changed_files_normalizes_repo_paths(); test_gold_evaluation_passes_matching_provider(); test_gold_evaluation_reports_quality_issues(); test_eval_report_roundtrip(); test_eval_comparison_ranks_providers(); test_gold_evaluation_reuses_candidate_cache_and_logs_requests(); test_eval_comparison_cache_key_uses_provider_spec_not_label(); test_sample_gold_cases_is_stratified_and_repeatable(); test_build_gold_cases_from_reference_tree(); test_write_gold_cases_roundtrip(); test_write_gold_review_pack_exports_review_files(); test_apply_gold_review_csv_preserves_original_case_structure(); test_match_terms_uses_source_and_variants(); test_audit_terms_reports_conflicts_and_invalid_rows(); test_merge_glossary_terms_later_inputs_override_same_source(); test_prepare_localize_update_writes_changed_files_and_source_baseline(); test_import_markdown_lore_cache_roundtrip_and_match(); test_import_json_lore_uses_anchors(); test_import_lore_directory_skips_readme(); test_match_lore_uses_ngram_similarity_without_anchor_hit(); test_lore_index_roundtrip_and_search(); test_evaluate_memory_retrieval_scores_fuzzy_matches_and_thresholds(); test_qa_detects_placeholder_mismatch(); test_qa_detects_traditional_and_length(); test_qa_uses_path_specific_length_policy(); test_qa_uses_display_width_policy(); test_translation_review_pack_exports_qa_and_apply_review_state(); test_extract_term_candidates_excludes_known_glossary(); test_rules_refiner_classifies_core_decisions(); test_refined_terms_cache_roundtrip(); test_refine_candidates_with_cache_reuses_existing_terms(); test_get_term_refiner_resolves_supported_providers(); test_promote_refined_terms_exports_only_confirmed_terms(); test_write_term_review_pack_exports_review_and_paratranz_files(); test_glossary_terms_from_review_csv_imports_only_approved_rows(); test_translate_appends_missing_data_list_record(); test_translate_reuses_candidate_cache_and_records_trace(); test_translate_skips_locked_unit(); test_state_apply_writes_reviewed_translation_without_provider(); test_state_status_reports_ready_and_pending_units(); test_translate_provider_receives_structured_context(); test_context_includes_cross_file_similar_memory(); test_source_changed_context_includes_previous_target_text(); test_openai_compatible_chat_provider_sends_structured_prompt(); test_qwen_mt_provider_uses_translation_options_without_system_message(); test_qwen_translation_options_tolerates_invalid_context(); test_get_provider_accepts_compatible_provider_specs(); test_get_provider_rejects_missing_compatible_model(); print('direct unit tests passed')"

smoke:
	rm -rf build
	mkdir -p build
	python3 -c "import json, pathlib, subprocess; repo=pathlib.Path('build/localize-repo'); repo.mkdir(); run=lambda *args: subprocess.run(['git', *args], cwd=repo, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True).stdout; run('init'); run('config','user.email','test@example.com'); run('config','user.name','Test User'); (repo/'KR').mkdir(); (repo/'LLC_zh-CN').mkdir(); json.dump({'dataList':[{'id':1,'desc':'예전 문장입니다.'}]}, open(repo/'KR'/'Sample.json','w',encoding='utf-8'), ensure_ascii=False); json.dump({'dataList':[{'id':1,'desc':'旧译文。'}]}, open(repo/'LLC_zh-CN'/'Sample.json','w',encoding='utf-8'), ensure_ascii=False); run('add','.'); run('commit','-m','base'); json.dump({'dataList':[{'id':1,'desc':'새로운 문장입니다.'}]}, open(repo/'KR'/'Sample.json','w',encoding='utf-8'), ensure_ascii=False); (repo/'README.md').write_text('docs\\n', encoding='utf-8'); run('add','.'); run('commit','-m','head'); print('localize git fixture ok')"
	python3 -m limbus_translate.cli localize prepare-update \
		--repo build/localize-repo \
		--base HEAD~1 \
		--head HEAD \
	...
	python3 -c "import json; rows=[{'provider':'local-reviewed','project_id':None,'term_id':100,'source_lang':'ko','target_lang':'zh-cn','source':'수감자','target':'囚徒','note':'review override','part_of_speech':'noun','variants':[],'case_sensitive':False,'created_at':None,'updated_at':'2026-06-14T00:00:00Z','raw':{},'fetched_at':'2026-06-14T00:00:00Z'},{'provider':'local-reviewed','project_id':None,'term_id':101,'source_lang':'ko','target_lang':'zh-cn','source':'지크프리트','target':'齐格弗里德','note':'reviewed name','part_of_speech':'noun','variants':[],'case_sensitive':False,'created_at':None,'updated_at':'2026-06-14T00:00:00Z','raw':{},'fetched_at':'2026-06-14T00:00:00Z'}]; json.dump(rows, open('build/glossary-overrides.json','w',encoding='utf-8'), ensure_ascii=False, indent=2); print('glossary override fixture ok')"
	python3 -m limbus_translate.cli glossary merge \
		--input build/glossary.json \
		--input build/glossary-overrides.json \
		--output build/glossary-merged.json
	python3 -c "import json; rows=json.load(open('build/glossary-merged.json', encoding='utf-8')); by_source={row['source']: row for row in rows}; assert len(rows) == 3; assert by_source['수감자']['target'] == '囚徒'; assert by_source['수감자']['provider'] == 'local-reviewed'; assert by_source['단테']['target'] == '但丁'; assert by_source['지크프리트']['target'] == '齐格弗里德'; print('glossary merge schema ok')"
	...
	python3 -m limbus_translate.cli state apply \
		...
		--output build/reviewed-output
	python3 -c "import json; data=json.load(open('build/reviewed-output/Sample.json', encoding='utf-8')); assert data['dataList'][0]['desc'] == '审校后的译文。'; print('state apply output schema ok')"
	...
	python3 -m limbus_translate.cli tm evaluate \
		...
		--include-exact
	python3 -c "import json; report=json.load(open('build/tm-eval-report.json', encoding='utf-8')); assert report['summary']['total'] == 2; assert report['summary']['with_match'] == 2; assert report['cases'][0]['matches']; print('tm eval schema ok')"
...
finalize-current-localize-review:
	bash scripts/finalize-current-localize-review.sh

apply-current-term-review:
	bash scripts/apply-current-term-review.sh

prepare-current-model-eval:
	bash scripts/prepare-current-model-eval.sh

apply-current-model-review:
	bash scripts/apply-current-model-review.sh

compare-current-models:
	bash scripts/compare-current-models.sh
