...
smoke:
	...
	python3 -m limbus_translate.cli workflow run \
		...
		--length-policy config/length-policy.sample.json \
		--provider dry-run
	python3 -c "import json; summary=json.load(open('build/workflow/summary.json', encoding='utf-8')); assert summary['units'] == 2; assert summary['translated'] == 2; assert summary['artifacts']['lore_index']; assert summary['artifacts']['glossary_audit']; assert summary['artifacts']['term_candidates']; assert summary['artifacts']['refined_terms']; assert summary['artifacts']['term_review_csv']; assert summary['artifacts']['translation_review_csv']; assert summary['artifacts']['translation_review_jsonl']; assert summary['glossary_audit']['total_terms'] == 2; assert summary['glossary_audit']['issues'] == 0; assert summary['translation_review']['selected'] == 2; assert summary['terms']['candidates'] == 3; assert summary['terms']['refined'] == 3; assert summary['qa_issues'] == 2; print('workflow summary schema ok')"
