...
smoke:
	...
	python3 -c "import json; cache=json.load(open('build/eval-candidates.json', encoding='utf-8')); requests=[json.loads(line) for line in open('build/eval-requests.jsonl', encoding='utf-8') if line.strip()]; compare_cache=json.load(open('build/eval-compare-candidates.json', encoding='utf-8')); compare_requests=[json.loads(line) for line in open('build/eval-compare-requests.jsonl', encoding='utf-8') if line.strip()]; report=json.load(open('build/eval-compare-report.json', encoding='utf-8')); assert cache and requests and all(row['provider'] == 'dry-run' for row in cache); assert all(row['target_text'] and 'usage' in row and row['response_model'] == 'dry-run' for row in requests); assert compare_cache and compare_requests; assert {row['provider'] for row in compare_cache} == {'dry-run'}; assert all(row['target_text'] and 'usage' in row for row in compare_requests); assert report['summary']['providers'] == 2; assert [row['provider'] for row in report['summary']['rankings']] == ['baseline','candidate']; assert len(report['providers']) == 2; print('eval cache, request log, and compare schema ok')"
