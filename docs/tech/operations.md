...
`make prepare-current-localize-review` 是面向真实上游更新的审校包入口。它会 checkout `LOCALIZE_COMMIT`（默认 `origin/main`）、同步 Paratranz 术语、以 GitHub `LLC_zh-CN` 为目标基线扫描 gap units，然后运行 `workflow run` 产出 `build/current-review/missing-units.json`、术语候选和 `translation-review/review.csv`。默认 `PROVIDER=dry-run` 只用于打通流程；需要真实候选时可设 `PROVIDER=qwen-mt`、`PROVIDER=openai` 或其他 provider。若需要只处理某次上游 diff，可设置 `LOCALIZE_BASE=<base-ref>`。
