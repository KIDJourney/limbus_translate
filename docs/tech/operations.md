...
`make prepare-current-localize-review` 是面向真实上游更新的审校包入口。它会 checkout `LOCALIZE_COMMIT`（默认 `origin/main`）、同步 Paratranz 术语、以 GitHub `LLC_zh-CN` 为目标基线扫描 gap units，然后运行 `workflow run` 产出 `build/current-review/missing-units.json`、术语候选和 `translation-review/review.csv`。如果存在 `cache/glossary/local-reviewed.json` 且未显式传入 `GLOSSARY_PATH`，脚本会把本地审校术语自动并入本轮 Paratranz cache 后用于 workflow。默认 `PROVIDER=dry-run` 只用于打通流程；需要真实候选时可设 `PROVIDER=qwen-mt`、`PROVIDER=openai` 或其他 provider。真实 provider 首跑建议设置 `LIMIT=1` 控制成本，例如 `LIMIT=1 PROVIDER=qwen-mt make prepare-current-localize-review`。若需要只处理某次上游 diff，可设置 `LOCALIZE_BASE=<base-ref>`。
...
`make apply-current-term-review` 是术语审校后的本地缓存入口。默认读取 `build/current-review/term-review/review.csv`，只把明确 approved 且有 target 的行写入 `cache/glossary/local-reviewed.json`，并与 Paratranz cache 合成为 `cache/glossary/active.json`，随后运行 glossary audit。该命令不会写回 Paratranz；下一轮 `make prepare-current-localize-review` 会默认读取 `cache/glossary/local-reviewed.json` 并并入本轮术语上下文，显式设置 `GLOSSARY_PATH` 时则由调用方控制术语来源。
