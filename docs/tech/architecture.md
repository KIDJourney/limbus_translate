...
LocalizeLimbusCompany checkout
  ...
  LLC_zh-CN/**/*.json
    ...
    -> cli.py workflow run: scan -> TM -> glossary audit -> term review pack -> lore index -> translate -> QA -> translation review summary
...
11. `workflow run` 把 scan、TM 构建、可选 glossary audit、术语候选提取/refine/review pack、可选 lore 导入/索引、translate、QA 和 translation review pack 串成一次可复现更新，输出工作目录内的 `missing-units.json`、`tm.json`、可选 `glossary-audit.json`、`term-candidates.json`、`refined-terms.json`、`term-review/`、可选 lore cache/index、`qa-report.json`、`translation-review/` 和 `summary.json`。
