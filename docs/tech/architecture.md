...
Limbus Translate 当前是一个 Python CLI 工具，围绕 LocalizeLimbusCompany 的 JSON 语言包做增量扫描、术语同步和候选译文生成。`KR` 是源文目录，GitHub `LLC_zh-CN` 是目标基线；工具默认保留已有中文，只对缺失、空值、仍韩文或源文变化的 JSON path 生成候选和补丁。
...
LocalizeLimbusCompany checkout
  ...
  LLC_zh-CN/**/*.json
    ...
    -> translator.py: overlay existing target baseline and set only selected translated JSON paths
...
| `limbus_translate/scanner.py` | 生成待翻译单元；以 `LLC_zh-CN` 为目标基线，跳过已有中文目标文本，只保留缺失、空值、仍等于源文、仍含可见韩文或 `source_changed` 的路径；支持唯一、非 `-1` 的 `dataList[*].id` 稳定对齐，重复/无效 id 回退 JSON path；支持 scan policy、changed-files 文件级增量扫描和 source-baseline JSON path 级源文变化扫描 |
...
| `limbus_translate/translator.py` | 从现有目标基线复制同结构 JSON 输出树，只把候选译文或 reviewed state 写回本轮 selected units；非 exact TM 命中时构建上下文包并传给 provider；对 `missing_target_record` 会复制源 record 到目标 `dataList` 后替换待译字段 |
...
2. `scan` 读取 `KR` 与 GitHub `LLC_zh-CN`，把 `LLC_zh-CN` 作为目标基线：已有中文且源文未变的路径不会进入待译列表，只有缺失、空值、仍等于源文、仍含可见韩文或源文变化的路径会输出为 `TranslationUnit[]`。可选读取 `--scan-policy` 作为文件类型 adapter 配置，也可读取 `--changed-files` 把范围收敛到本次 git diff 涉及的 JSON 文件；如果传入 `--source-baseline`，会按 JSON path 和 `dataList[id=...]` 稳定键只保留源文新增/变化的路径，并把目标已有旧中文的单元标记为 `source_changed`。输出单元包含 `source_json_path`、目标 `json_path`、`stable_key`、source hash 和格式 profile。
...
13. 审校通过后，`workflow finalize` 用 reviewed / locked state 在 `LLC_zh-CN` 目标基线上生成发布候选目录，写出 `state-status.json`、`qa-report.json` 和 `summary.json`，并可用 `--fail-if-pending` / `--fail-on-error` 做发布前门禁；启用 Localize patch 输出时，patch 应只包含 reviewed units 对应的替换，不包含全量重排或已有中文重写。确认后的译文可继续进入 TM 和回归评估集。
...
| 目标基线优先 | GitHub `LLC_zh-CN` 是目标基线；已有中文默认保留，只对缺失、空值、仍韩文或 `source_changed` 单元生成候选和 patch |
