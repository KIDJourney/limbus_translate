...
LocalizeLimbusCompany checkout
  ...
  LLC_zh-CN/**/*.json
    ...
    -> cli.py workflow run: scan -> TM -> term review pack -> lore index -> translate -> QA summary
...
| `limbus_translate/cli.py` | 命令行入口；`workflow run` 串联扫描、TM、术语候选/审校包、lore index、翻译输出、QA 和 summary |
...
10. `workflow run` 把 scan、TM 构建、术语候选提取/refine/review pack、可选 lore 导入/索引、translate 和 QA 串成一次可复现更新，输出工作目录内的 `missing-units.json`、`tm.json`、`term-candidates.json`、`refined-terms.json`、`term-review/`、可选 lore cache/index、`qa-report.json` 和 `summary.json`。
