...
| 日期 | 场景 | 变更 | 验证 | 结果 |
|---|---|---|---|---|
| 2026-06-13 | Limbus Translate 初版工具 | 新增 Python CLI、JSON 扫描、Paratranz 术语同步、dry-run 翻译输出 | fixture scan；直接调用测试函数；`glossary sync-paratranz`；`translate --provider dry-run`；真实 Localize checkout 扫描 | 通过；fixture 扫描 2 条；Paratranz 同步 1963 条；dry-run 输出同结构 JSON；真实扫描 263 条 `target_same_as_source`，仍需 adapter 区分内部标识 |
