...
| 日期 | 场景 | 变更 | 验证 | 结果 |
|---|---|---|---|---|
| 2026-06-13 | Limbus Translate 初版工具 | 新增 Python CLI、JSON 扫描、Paratranz 术语同步、TM、dry-run 翻译输出、QA | `make test`；`make smoke`；`make validate-docs`；`compileall`；`glossary sync-paratranz`；真实 Localize checkout 扫描；真实 TM 构建 | 通过；fixture 扫描 2 条；Paratranz 同步 1963 条；dry-run 输出同结构 JSON；真实默认扫描 19 条，高风险 `StoryData/*.content`；真实 TM 构建 92337 条 |
