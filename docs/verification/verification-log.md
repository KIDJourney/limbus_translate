...
| 日期 | 场景 | 变更 | 验证 | 结果 |
|---|---|---|---|---|
| 2026-06-13 | Limbus Translate 初版工具 | 新增 Python CLI、JSON 扫描、唯一 `dataList.id` 对齐、Paratranz 术语同步、TM、dry-run 翻译输出、QA、术语候选缓存 | `make test`；`make smoke`；`make validate-docs`；`compileall`；`glossary sync-paratranz`；真实 Localize checkout 扫描；真实 TM 构建；`terms extract` | 通过；fixture 扫描 2 条；数组顺序变化测试通过；Paratranz 同步 1963 条；dry-run 输出同结构 JSON；真实默认扫描 19 条，高风险 `StoryData/*.content`；真实 TM 构建 92337 条；真实术语候选 19 条 |
...
1. 缺失 `dataList` record 目前只报告为 `missing_target_record`，尚未自动生成整条目标记录。
2. QA 尚未覆盖简繁、UI 长度、MQM 分类和 reviewed / locked 状态。
3. 术语候选提取仍是 heuristic 召回，正式术语需要 LLM 或人工二次筛选。

## 未覆盖项

- CI 文档检查。
- LLM 术语提炼 provider。
- reviewed / locked 状态和 fuzzy TM。
