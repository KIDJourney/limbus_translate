...
| CLI smoke | fixture 扫描、TM 构建、state 初始化、dry-run 同结构输出、QA 报告、术语候选缓存 | `make smoke` |
...
2. `make test` 通过。
3. `make smoke` 通过，输出 `build/missing-units.json`、`build/tm.json`、`build/state.json`、`build/LLC_zh-CN/Sample.json`、`build/qa-report.json` 和 `build/term-candidates.json`。
