...
- 修改 `cli.py` 的 `workflow run` / `workflow finalize` 或其串联模块：执行 `make test`、`make smoke`、`make validate-docs` 和至少一次真实 checkout 小范围 workflow。
