...
| CLI smoke | 带 scan policy 和 changed-files 的 fixture 扫描、TM 构建、glossary audit、lore import、lore index/search、state 初始化、带 lore index 的 dry-run 同结构输出、candidate cache、translation trace、带 length policy 的 QA 报告、translation review pack/apply、gold set 构建、gold sample、gold review pack、curated gold、eval report、eval compare report、术语候选缓存、rules 二次提炼缓存、review pack、review apply、promoted glossary cache，以及带 glossary audit / candidate cache / trace 的 `workflow run` 端到端 summary 和术语审校包 artifact | `make smoke` |
