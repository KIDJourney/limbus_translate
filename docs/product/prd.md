...
| Gold set / 模型评估 | 用固定样本评估 provider 翻译质量和 prompt 变更风险 | 已完成从参考译文构建 gold set、分层采样、人工审校回写、单 provider eval report、多 provider compare report、eval candidate cache 和 eval request log 初版；真实模型赛马未完成 |
